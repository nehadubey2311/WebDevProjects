...
function editPost() {
  ...
  const content = document.querySelector(`.content[data-post-id="${postId}"]`)
    .innerHTML;

  // display edit textarea pre-filled with current content and save button
  document.querySelector(`.content[data-post-id="${postId}"]`).style.display =
    "none";
  ...
  document
    .querySelector(`#editBtn-${postId}`)
    .addEventListener("click", () => savePost(postId));
}
...
function likeUnlikePost() {
  let postId = this.dataset.like;
  fetch(`/post/${postId}/like_unlike_post`, {
    method: "POST",
    body: JSON.stringify({
      postId: postId,
    }),
